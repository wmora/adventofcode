...
const data = fs.readFileSync('day4input.txt', {encoding:'utf8', flag:'r'}).split('\n').filter(Boolean);

let sum = 0;

const isAssignmentContained = function(firstAssignment, secondAssignment) {
    ...
    const isLowestSectionContained = lowestSectionFirstAssignment >= lowestSectionSecondAssignment 
        && lowestSectionFirstAssignment <= highestSectionSecondAssignment;

    const isHighestSectionContained = highestSectionFirstAssignment >= lowestSectionSecondAssignment 
...
data.forEach(line => {
    ...
    if (isAssignmentContained(firstAssignment, secondAssignment) || isAssignmentContained(secondAssignment, firstAssignment)) {
        sum++;
    }
});
