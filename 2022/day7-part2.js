...
const data = fs
  .readFileSync('day7input.txt', { encoding: 'utf8', flag: 'r' })
  .split('\n')
  .filter(Boolean);
...
node.reverseTraverse((node) => {
  if (node.isDirectory) {
    node.getChildren().forEach((child) => {
      if (
        child.size >= neededSpace &&
        child.isDirectory &&
        child.size < smallestDirectorytoDelete.size
      ) {
...
console.log(
  smallestDirectorytoDelete.getName(),
  smallestDirectorytoDelete.size
);
