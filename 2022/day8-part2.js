...
const data = fs
  .readFileSync('day8input.txt', { encoding: 'utf8', flag: 'r' })
  .split('\n')
  .filter(Boolean);
...
for (let i = 0; i < map.length; i++) {
  ...
  for (let j = 0; j < row.length; j++) {
    ...
    let leftFirstBlockingTreeIndex =
      row
        .slice(0, j)
        .reverse()
        .findIndex((height) => height >= treeHeight) + 1;
    ...
    let rightFirstBlockingTreeIndex =
      rightRow.findIndex((height) => height >= treeHeight) + 1;
    ...
    let bottomFirstBlockingTreeIndex =
      bottomColumn.findIndex((height) => height >= treeHeight) + 1;
    ...
    const scenicScore =
      leftFirstBlockingTreeIndex *
      rightFirstBlockingTreeIndex *
      topFirstBlockingTreeIndex *
      bottomFirstBlockingTreeIndex;
